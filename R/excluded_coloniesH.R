#' Neighborful algorithm - Variation-H
#'
#' The variation-H of neighborful algorithm uses only \code{param1_threshold}
#' exclusion criterion to remove colonies as excluded ones by removing
#' \code{empty_indices} from both outer and inner loops of the algorithm. The
...
#' excluded_coloniesH(plateformat = plateformat,
...
#'                    param1_threshold = 1,
#'                    is_middle = TRUE)
excluded_coloniesH <- function(plateformat,
                               colony_indices,
    ...
    if(colony_area_raw_data[colony] <= plate_median_threshold){
      if(! colony %in% empty_indices){
        ...
        if(max_kernel_density >= param1_threshold) {
          ...
          # apply variation-4
          excluded_colonies <- variation4(colony = colony,

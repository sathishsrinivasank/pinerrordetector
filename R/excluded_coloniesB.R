#' Neighborful algorithm - Variation-B
#'
#' The variation-B of neighborful algorithm uses both \code{param1_threshold} and
#' \code{param2_threshold} exclusion criteria to remove colonies as excluded ones
#' by not removing \code{empty_indices} from both outer and innner loops of the
#' algorithm. The outer loop of the algorithm is used to iterate over the list
#' of \code{colony indices}. The inner loop of the algorithm is used to iterate
...
#' excluded by the previous iteration of neighborful algorithm of variation-B.
...
#' excluded_coloniesB(plateformat = plateformat,
...
#'                    param2_threshold = 1,
#'                    is_middle = TRUE)
excluded_coloniesB <- function(plateformat,
                               colony_indices,
  ...
  for (colony in colony_indices) {
    if(colony_area_raw_data[colony] <= plate_median_threshold){
      ...
      if(max_kernel_density >= param1_threshold) {
        ...
        # apply variation-2
        excluded_colonies <- variation2(colony,
                                        combin_coords,
